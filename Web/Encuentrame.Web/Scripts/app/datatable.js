...
//var getRelatedTable = function ($filterObject) {
//    var $filtersContainer = $filterObject.parents(".data-table-filters:first");
//    var relatedTableId = $filtersContainer.data("table-id");
//    var forChildTable = $filterObject.parents(".table-filter-container:first").data("for-child-table");

//    if (forChildTable) {
//        var dataChildProperty = $filtersContainer.data("child-property");
//        var detailsName = relatedTableId + "_Details_";
//        var $detailsTables = $('table[id^=' + detailsName + dataChildProperty + '_]:not([id$="_wrapper"])');
//        return $detailsTables;
//    }

//    var $relatedTable = $("#" + relatedTableId);
//    return [$relatedTable];
//}

function getIndex(jqueryObj, hasDetails) {
...
function processTable(table, jsonData, addChildTable, addFilters, isChildTable) {
    ...
    $ths.each(function (index, th) {
        ...
        var columnIndex = getIndex($th, hasDetails);
...
var setupTable = function ($dom, table, jsonData) {
    ...
    function addChildTable(childTableInfo) {
        ...
        $(rootTableData.currentChildrenFilters).each(function (idx, obj) {
            ...
            currentFilter.filterTable(childTableInfo.jqueryChildTable, childTableInfo.hasDetails);
        });
        ...
        if (relatedTableId === rootTableData.tableId) {
            ...
            $.ajax({
                ...
                var selectFilterObj = {
                    ...
                    IsRowMatch: function (tableToFilter, hasDetails, searchData, rowData) {
                ...
                if (isMultiple) {
                    selectFilterObj.IsRowMatch = function (tableToFilter, hasDetails, searchData, rowData) {
                        var currentInputFilterObject = this;
                        var returnValue = true;
    ...
    $dom.find(".date-range-filter").each(function (idx, obj) {
        ...
        if (relatedTableId === rootTableData.tableId) {
            ...
            var dateRangeFilterObj = {
                ...
                IsRowMatch: function (tableToFilter, hasDetails, searchData) {
                    var currentInputFilterObject = this;
                    var returnValue = true;
    ...
    $dom.find('.select-enum-filter').each(function (idx, obj) {
        ...
        if (relatedTableId === rootTableData.tableId) {
            ...
            var enumFilterObj = {
                ...
                filterTable: function (tableToFilter, hasDetails, triggerDraw) {
            ...
            if (searchValue !== "" && searchValue !== undefined) {
                if (!enumFilterObj.forChildTable) {
                    ...
                    enumFilterObj.filterTable(rootTableData.jqueryTable, rootTableData.hasDetails, false);
                } else {
            ...
            $enumFilter.on('change', function () {
                ...
                if (enumFilterObj.forChildTable) {
                    ...
                    $.each(rootTableData.childTables, function (key, value) {
                        var tableData = value;
                        enumFilterObj.filterTable(tableData.table, tableData.hasDetails, !rootTableData.deferredDraw);
                    });
                    rootTableData.filterChildren = true;
                } else {
                    if ($.inArray(enumFilterObj, rootTableData.currentFilters) === -1)
                        rootTableData.currentFilters.push(enumFilterObj);
                    enumFilterObj.filterTable(rootTableData.jqueryTable, rootTableData.hasDetails, !rootTableData.deferredDraw);
                    rootTableData.filterParent = true;
                }
            });
        }
        ...
        if (relatedTableId === rootTableData.tableId) {
            ...
            if (defaultData !== "" && defaultData !== undefined) {
                if (!booleanFilterObj.forChildTable) {
                    ...
                    booleanFilterObj.filterTable(rootTableData.jqueryTable, rootTableData.hasDetails, false);
                } else {
        ...
        if (relatedTableId === rootTableData.tableId) {
            ...
            if (searchValue !== "" && searchValue !== undefined) {
                if (!inputFilterObj.forChildTable) {
                    ...
                    inputFilterObj.filterTable(rootTableData.jqueryTable, rootTableData.hasDetails, false);
                } else {
            ...
            $inputFilter.on('keyup', function () {
                ...
                if (inputFilterObj.searchValue !== $filter.val()) {
                    ...
                    if (inputFilterObj.forChildTable) {
                        ...
                        $.each(rootTableData.childTables, function (key, value) {
                            var tableData = value;
                            inputFilterObj.filterTable(tableData.table, tableData.hasDetails, !rootTableData.deferredDraw);
                        });
                        rootTableData.filterChildren = true;
                    } else {
                        if ($.inArray(inputFilterObj, rootTableData.currentFilters) === -1)
                            rootTableData.currentFilters.push(inputFilterObj);
                        inputFilterObj.filterTable(rootTableData.jqueryTable, rootTableData.hasDetails, !rootTableData.deferredDraw);
                        rootTableData.filterParent = true;
                    }
                }
            });
           ...
           function (settings, searchData, index, rowData, counter) {
               ...
               if (currentId in rootTables) {
                   ...
                   $(tableToFilter.currentNotRegularFilters).each(function (idx, obj) {
                       var currentFilter = this;
                       returnValue = returnValue && currentFilter.IsRowMatch(tableToFilter.jqueryTable, tableToFilter.hasDetails, searchData, rowData);;
                   });
               } else {
                   ...
                   $.each(rootTables, function (key, value) {
                       if (currentId.match("^" + key)) {
                           ...
                           $(rootTable.currentNotRegularChildrenFilters).each(function (idx, obj) {
                               var currentFilter = this;
                               returnValue = returnValue && currentFilter.IsRowMatch(childTableData.table, childTableData.hasDetails, searchData, rowData);
                           });
                           return false;
                       }
                   });
